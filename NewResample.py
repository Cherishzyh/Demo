...
if __name__ == '__main__':
    ...
    resamper = Resampler()
    raw_folder = r'D:\Data\Zhengzhou1st_DXI_All'
    for folder in os.listdir(raw_folder):
        case_folder = os.path.join(raw_folder, folder)
        if not os.path.isdir(case_folder): continue
        image, image_array, _ = LoadImage(os.path.join(case_folder, 'SMS3diffDKI6biso22mm_bneddy_b0_Reg.nii.gz'), is_show_info=False)
        roi, roi_array, _ = LoadImage(os.path.join(case_folder, 'TRAS.nii.gz'), is_show_info=False)
        try:
            roi_Reg = resamper.ResampleToReference(roi, image, sitk.sitkLinear,
                                                   save_path=os.path.join(case_folder, 'TRAS_Reg.nii.gz'), is_roi=True)
            print('Successfully Resample: {}'.format(folder))
        except Exception as e:
            print(e)


    # roi_array_Reg = sitk.GetArrayFromImage(roi_Reg).transpose(1, 2, 0)
    # Imshow3DArray(Normalize01(image_array), roi=Normalize01(np.flip(roi_array_Reg, axis=0)))
    # print()
